...
Transition = namedtuple("Transition", ("state", "reward", "action", "c_flag"))

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
eps = 1e-10
_eps = torch.tensor(eps, device=device)
...
class RandomShiftsAug(nn.Module):
    ...
    def forward(self, x):
        ...
        arange = torch.linspace(
            -1.0 + eps, 1.0 - eps, w + 2 * self.pad, device=x.device, dtype=x.dtype
        )[:w]
        arange = arange.unsqueeze(0).repeat(h, 1).unsqueeze(2)

        eps = 1.0 / (h + 2 * 4)
        arange2 = torch.linspace(
            -1.0 + eps, 1.0 - eps, h + 2 * 4, device=x.device, dtype=x.dtype
        )[:h]
...
class PrioritizedReplay_nSteps_Sqrt(object):
    ...
    def reset_last_k(self, current_step, k=30000):
        ...
        self.priority = torch.cat(
            (self.priority, torch.zeros(self.capacity - k, dtype=torch.float))
        )
