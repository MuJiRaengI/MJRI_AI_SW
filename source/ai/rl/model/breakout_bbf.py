...
class BBF_Model(nn.Module):
    def __init__(
        ...
        self.transition = nn.Sequential(
            ...
            DQN_Conv(
                32 * scale_width, 32 * scale_width, 3, 1, 1, norm=False, act=self.act
            ),
        )
    ...
    def encode(self, X):
        ...
        self.seq = seq
        X = self.encoder_cnn(
            X.contiguous().view(self.batch * self.seq, *(X.shape[2:]))
        ).contiguous()
    ...
    def get_transition(self, z, action):
        ...
        action = action.view(-1, 5, self.n_actions, 1, 1).expand(
            -1, 5, self.n_actions, *z.shape[-2:]
        )
    ...
    def network_ema(self, rand_network, target_network, alpha=0.5):
        for param, param_target in zip(
            rand_network.parameters(), target_network.parameters()
        ):
            param_target.data = (
                alpha * param_target.data + (1 - alpha) * param.data.clone()
            )

    def hard_reset(self, random_model=None, alpha=0.5):
        if random_model == None:
            random_model = BBF_Model(
                self.n_actions, self.hiddens, self.scale_width, self.num_buckets
            ).cuda()
