...
class Env2048(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=0, high=2**17, shape=(size, size), dtype=np.int32
        )
    ...
    def step(self, action: int):
        ...
        if self.reward_mode == "merge_sum":
            reward = float(merged_value) * self.reward_scale
            if merged_value > 1024:
                print(f"Merged value: {merged_value}, now board:\n{self.board}")
...
# --------------------- 간단 실행기 ---------------------
def _autoplay_step(env: Env2048):
