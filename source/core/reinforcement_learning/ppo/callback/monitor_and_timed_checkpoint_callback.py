...
class MonitorAndTimedCheckpointCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        if self.save_interval_sec is not None:
            ...
            if self.verbose:
                print(
                    f"[Callback] Checkpoints -> {self._save_path}.zip (every {self.save_interval_sec}s)"
                )
    ...
    def _save_latest(self, suffix: str = "") -> None:
        ...
        """
        base: Path = (
            self._save_path if suffix == "" else Path(str(self._save_path) + suffix)
        )
        self.model.save(str(base))  # SB3 save는 str path 필요
        if self.verbose:
            print(
                f"[Checkpoint] Saved -> {base}.zip @ step={self.n_calls}, timesteps={self.num_timesteps}"
            )
