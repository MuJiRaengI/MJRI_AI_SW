...
class GA2048(GeneticAlgorithm):
    ...
    def learn(self, config: dict):
        ...
        hist_dir = os.path.join(self.save_dir, "history")
        ...
        for g in range(generations):
            # 3-1) 평가
            fits = []
            now_hist = {"gen": g + 1, "w_list": pop.tolist(), "seed": seed}
            for idx, w in enumerate(tqdm(pop, desc=f"Gen {g+1}/{generations} Eval")):
                fit, h = self.fitness(
                    w,
                    eval_episodes,
                    max_steps,
            ...
            now_hist["best_fit"] = best_fit
            now_hist["avg_fit"] = float(np.mean(fits))
            now_hist["sigma"] = sigma
            # now_hisroy json save
            with open(os.path.join(hist_dir, f"gen_{g+1:04d}.json"), "w") as f:
                json.dump(now_hist, f, indent=4)
    ...
    def fitness(
        ...
        for ep in range(eval_episodes):
            s, mt, h = self.rollout(w, max_steps, invalid_move_penalty, seed=ep)
            scores.append(s)
            tiles.append(mt)
            hist["ep" + str(ep)] = h.tolist()
        bonus = bonus_2048 * sum(mt >= 2048 for mt in tiles)
        fit = float(np.mean(scores)) + bonus
        return fit, hist
